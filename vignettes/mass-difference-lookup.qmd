...
[*Isotope fine structure*](isotope-fine-structure.html) starts from a candidate
formula. Most of the time you don't have one yet: you have two peaks in a spectrum
and a delta between them, and the question is "what family of thing produces a gap
this size?" An in-source fragment loses a neutral (water, CO₂, a whole side chain).
A homologous series steps by a repeating unit (CH₂, C₂H₄O, a whole PEG monomer).
An adduct swaps one attached ion for another (Na⁺ for K⁺, formate for acetate). All
three are catalogued in commonMZ already: `adducts_fragments` and
`repeating_units_pos`/`repeating_units_neg`, merged by `mz_diff_table()` into one
